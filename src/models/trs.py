...
class TRSDatabaseBuilder:
    ...
    def update_trs_database(self, days_back: int = 30, verbose: bool = True):
        """
        Update TRS database with recent data only
        """
        raw_db = CDSDatabase(self.raw_db_path)
        trs_conn = sqlite3.connect(self.trs_db_path)
        ...
        for index_name, index_info in self.indices.items():
            for tenor in self.tenors:
                ...
                try:
                    # Check last date in TRS database
                    try:
                        last_date_query = f"SELECT MAX(date) as last_date FROM {table_name}"
                        result = trs_conn.execute(last_date_query).fetchone()
                    ...
                    # Get new raw data
                    new_data = raw_db.query_historical_spreads(
                        index_name, tenor, start_date
                    )
                    
                    if new_data.empty:
                        if verbose:
                            print(f"  {index_name} {tenor}: No new data")
                        continue
                    
                    # Calculate TRS for new data
                    new_trs = self.calculate_trs(
                        new_data, 
                        tenor,
                        recovery_rate=index_info['recovery']
                    )
                    ...
                    new_trs.to_sql(table_name, trs_conn, 
                                if_exists='append', index=False)
                    
                    updated_count += 1
                    
                    if verbose:
                        print(f"  {index_name} {tenor}: Added {len(new_trs)} records")
                        
                except Exception as e:
                    failed_updates.append(f"{index_name}_{tenor}")
                    if verbose:
                        print(f"  {index_name} {tenor}: Failed - {e}")
        
        trs_conn.commit()
        trs_conn.close()
        raw_db.close()
        
        print(f"\nUpdate complete: {updated_count} series updated")
        if failed_updates:
